...
- **Parallel runs use per-slot environments, no locks**: `_run_parallel` lends each running job a slot index (0..N-1) from a queue; the environment directory is `<env_key>` for slot 0 (shared with sequential runs) and `<env_key>-<slot>` above (`cli._env_dir_name`). Concurrent jobs therefore never target the same directory, so same-`env_key` jobs run fully in parallel with no per-key lock and no sync/run split (issue #24). uv's hardlink cache keeps the duplicate environments cheap; the cost is N-fold directories, accepted deliberately.
